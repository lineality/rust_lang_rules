...
- Single-Flat: Default to a single-flat-file module x.rs, where the create contains two files: main.rs and x.rs. ,main.rs  uses "mod x; use x::FUNCTIONNAME;" to run the module. The single flat file "module" x.rs (where x = the name of the module) is then portable to be added to other crates and projects).
